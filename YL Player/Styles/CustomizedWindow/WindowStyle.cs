...
namespace YL_Player.Styles.CustomizedWindow
{
    ...
    public partial class WindowStyle
    {
        ...
        void IconMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
        {
            clickOnIcon++;
            clickOnTitleBar = 0;
            sender.ForWindowFromTemplate(w => (w as MainWindow).MainMenu.Visibility = Visibility.Visible);
            /*if (e.ClickCount > 1)
            {
        ...
        {
            if (clickOnIcon > 0 && clickOnTitleBar > 0)
            {
                clickOnIcon = 0;
                sender.ForWindowFromTemplate(w =>
                {
                    if ((w as MainWindow).MainMenu.Visibility == Visibility.Visible)
                        (w as MainWindow).MainMenu.Visibility = Visibility.Hidden;
                });
            }
            else
                clickOnTitleBar++;
